...
import weka.classifiers.trees.J48impreciserepTree2ajustadoPvariableS;
...
public class RAkEL extends MultiLabelMetaLearner {
    ...
    protected void buildInternal(MultiLabelInstances trainingData) throws Exception {
        ...
        sizeOfSubset = numLabels/2;

        // default number of models = twice the number of labels
        if (numOfModels == 0) {
            numOfModels = Math.min(2 * numLabels, 100);
        }
        classIndicesPerSubset = new int[numOfModels][sizeOfSubset];
    ...
    /*	Experimentation experimentation;
    	...
    	String file_results = location + "/RAkEL";
		int num_folds = 5;
		int num_learners = 3;
		...
		LabelPowerset LP1 = new LabelPowerset(base1);
		LabelPowerset LP2 = new LabelPowerset(base2);
		LabelPowerset LP3 = new LabelPowerset(base3);
		MultiLabelLearner learner1 = new RAkEL(LP1);
		MultiLabelLearner learner2 = new RAkEL(LP2);
		MultiLabelLearner learner3 = new RAkEL(LP3);
		learners[0] = learner1;
		learners[1] = learner2;
		learners[2] = learner3;
