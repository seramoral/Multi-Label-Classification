...
public class ConditionalEntropy2_ClassifierChain extends TransformationBasedMultiLabelLearner{
	/**
	 * For serialization
	 */
    ...
    /**
     * The conditional entropies for all the labels
     */
     ...
     */
    public ConditionalEntropy2_ClassifierChain() {
        super(new J48());
    }
     ...
     */
    public ConditionalEntropy2_ClassifierChain(Classifier classifier) {
        super(classifier);
    }
    ...
    protected double[] computeSumConditionedEntropies(){
    	...
    	for(int i = 0; i < numLabels; i++) {
    		...
    		for(int j = 0; j < numLabels; j++) {
    			if(j != i) { 
    				conditioned_entropy = conditional_entropies[i][j];
    ...
    protected void optimizeLaberingOrderChain(MultiLabelInstances train) {
    	...
    	double max_sum_entropies, sum_conditioned_entropies, conditioned_entropy;
    	int index_max_sum_entropies;
    	int temp;
    	...
    	for(int i = numLabels-1; i >=0; i--) {
    		...
    		Select the j = 0,..,q such that chain[j] = argmax_{1,..,i} sum_conditioned_entropies
    		*/
    		index_max_sum_entropies = 0;
    		max_sum_entropies = Double.NEGATIVE_INFINITY;
    		
    		for(int j = 0; j <= i; j++) {
    			sum_conditioned_entropies = sums_conditioned_entropies[chain[j]];
    			
    			if(sum_conditioned_entropies > max_sum_entropies) {
    				index_max_sum_entropies = j;
    				max_sum_entropies = sum_conditioned_entropies;
    			}
    		}
    		...
    		temp = chain[i];
    		chain[i] = chain[index_max_sum_entropies];
    		chain[index_max_sum_entropies] = temp;
    		
    		/* Update the sum of the conditioned entropies for the elements not yet inserted at the end of the chain
    		 * for j = 0,...,i, resting H(l_{chain(j)}|l_{chain(i)})
    		 */
    		
    		for(int j = 0; j <= i; j++) {
    			conditioned_entropy = conditional_entropies[chain[j]][chain[i]];
